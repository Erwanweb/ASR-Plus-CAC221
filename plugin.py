...
<plugin key="AC-ASR-CAC221" name="AC Aircon Smart Remote PLUS for CAC221" author="MrErwan" version="0.1.1" externallink="https://github.com/Erwanweb/ASR-Plus-CAC221.git">
    ...
    <params>
        ...
        <param field="Mode5" label="Day/Night Activator, Pause On delay, Forced Eco Off delay (0=not timed), Presence On delay, Presence Off delay (all in minutes), reducted T(in degree), Delta max fanspeed (in in tenth of degre)" width="200px" required="true" default="0,1,1,2,45,3,5"/>
...
class BasePlugin:
    ...
    def onHeartbeat(self):
        ...
        self.pluginsetpoint = float(Devices[5].sValue)
            ...
            if self.ModeAuto: # Auto Mode
                ...
                if self.PresenceTH: # We use normal setpoint for confort
                    ...
                    if self.Turbopower: # Turbopower is on
                     ...
                     # check if setpoint is over and need adjustment
                     #if self.intemp > (float(Devices[5].sValue) + 1):
                        #self.overset = (self.intemp - float(Devices[5].sValue)
                        #Domoticz.Debug("AC set present overheating setpoint of '{}' - Setpoint is '{}' for Room Temp '{}'".format(self.overset, self.setpoint, self.intemp))
                        #self.septpoint = (float(Devices[5].sValue) - self.overset):
                        #if self.setpoint < 17:  # Setpoint Lower than range 17 to 30
                        #   self.setpoint = 17.0  # AC setpoint = Thermostat setpoint reducted in limit of range
                        #Domoticz.Debug("New Setpoint after correction is '{}'".format(self.setpoint))

                else: # No presence detected so we use reducted setpoint
                    ...
                    if not Devices[3].sValue == "10":
                        Devices[3].Update(nValue=self.powerOn, sValue="10")
                        Domoticz.Debug("Turn Fan speed in auto because reducted setpoint in AUTOMode")
                    if not self.WACfanspeedvalue == 10:
                        for idx in self.WACfanspeed:
                            DomoticzAPI("type=command&param=switchlight&idx={}&switchcmd=Set Level&level=10".format(idx))
                            Domoticz.Debug("WACfanspeed isn't at good Level at '{}'- Updating AC fanspeed widget Level at '10'".format(self.WACfanspeedvalue))
