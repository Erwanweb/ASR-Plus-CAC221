...
# MANUAL MODE ----------------------------------------------------------------------------------------------------------
        ...
        if devicesAPI:
            for device in devicesAPI["result"]:  # parse the devices for temperature sensors
                ...
                if idx in self.InTempSensors:
                    if "Temp" in device:
                        last_update_str = device.get("LastUpdate", None)
                        outdated = False
                        if last_update_str:
                            try:
                                last_update = datetime.strptime(last_update_str, "%Y-%m-%d %H:%M:%S")
                                if datetime.now() - last_update > timedelta(minutes=60):
                                    outdated = True
                            except Exception as e:
                                Domoticz.Error(f"Error reading LastUpdate on {device['Name']}: {e}")
                                outdated = True
                        if not device.get("HaveTimeout", False) and not outdated:
                            # Capteur OK
                            listintemps.append(device["Temp"])
                            Domoticz.Debug("device: {}-{} = {}".format(device["idx"], device["Name"], device["Temp"]))
                        else:
                            Domoticz.Error("device: {}-{} is not a Temperature sensor or is TimedOut".format(device["idx"], device["Name"]))
    ...
    def PresenceDetection(self):
        ...
        if self.ForcedEco or Parameters["Mode3"] == "":
            ...
            from datetime import datetime, timedelta  # à placer en haut du fichier

            from datetime import datetime, timedelta  # (si ce n'est pas déjà fait)
            ...
            if devicesAPI:
                for device in devicesAPI["result"]:
                    ...
                    if idx in self.DTpresence:  # this is one of our DT
                        ...
                        skip = False
                        dummy = True
                        # Vérifier si le device n'est PAS un "Dummy"
                        if device.get("HardwareName") != "Dummies":
                            dummy = False
                            # Vérifier si HaveTimeout ou LastUpdate > 30 min
                            if device.get("HaveTimeout", False):
                                ...
                                if last_update_str:
                                    try:
                                        ...
                                        Domoticz.Error("Error LastUpdate parsing pour '{}': {}".format(device["Name"], e))
                                        skip = True
                        if not skip and "Status" in device:
                            ...
                            if not dummy :
                                ...
                                Domoticz.Debug("Dummy switch idx '{}' - '{}' currently is '{}'".format(idx, device["Name"],device["Status"]))
                            if device["Status"] == "On":
                                self.DTtempo = datetime.now()
                        else:
                            Domoticz.Error("Device with idx '{}' named '{}' seem not DT or it is TimedOut!".format(idx, device["Name"]))
