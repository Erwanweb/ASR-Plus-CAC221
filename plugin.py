...
class BasePlugin:
    ...
    def onHeartbeat(self):
            ...
            if self.ModeAuto: # Auto Mode
                ...
                if self.PresenceTH: # We use normal setpoint for confort
                    ...
                    if self.Turbofan : # Turbofan is on
                        Domoticz.Debug("AUTOMode - Turbofan : Fan speed high because room temp is lower more than delta min from setpoint")
                    ...
                    if self.Turbopower: # Turbopower is on
                        self.setpoint = 30.0
                        Domoticz.Debug("AUTOMode - Turbopower : used setpoint is Max '30' because room temp is lower more than delta min from setpoint")
        ...
        # Log
        if self.powerOn:
        Domoticz.Log("System ON - Room Temp is '{}ºC for a Setpoint of '{}' and a Aircon Setpoint of '{}' ".format(self.intemp, self.pluginsetpoint, self.setpoint))
        else:
                ...
                for device in devicesAPI["result"]:  # parse the presence/motion sensors (DT) device
                    ...
                    if idx in self.DTpresence:  # this is one of our DT
                        if "Status" in device:
                            ...
                            Domoticz.Debug("DT switch with idx '{}' named '{}' currently is '{}'".format(idx, device["Name"], device["Status"]))
                            if device["Status"] == "On":
                                self.DTtempo = datetime.now()
