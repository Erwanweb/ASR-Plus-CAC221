...
class BasePlugin:
        ...
        if not self.powerOn :
            ...
            if self.ModeAuto :
                if self.PresenceTH :
                    if self.intemp < (float(Devices[5].sValue) - ((self.deltamax / 10) + (self.deltamax / 20))):
                        ...
                        if not self.WACfanspeedvalue == 40:
                            for idx in self.WACfanspeed:
                                ...
                                self.WACfanspeedvalue = self.WACfanspeed
                    else :
                    ...
                    if not self.WACsetpointvalue == str(self.setpoint):
                    ...
                    if not self.WACfanspeedvalue == 10:
                        for idx in self.WACfanspeed:
                            DomoticzAPI("type=command&param=switchlight&idx={}&switchcmd=Set%20Level&level=10".format(idx))
                            self.WACfanspeedvalue = self.WACfanspeed
    ...
    #def CAC221widgetcontrol(self):
        ...
        # check fan
        #APIjson2 = DomoticzAPI("type=devices&rid={}".format(self.WACfanspeed))
        #if APIjson2:
           #for device in APIjson2["result"]:  # check if idx is ok
