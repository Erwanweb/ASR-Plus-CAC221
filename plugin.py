...
class BasePlugin:
    ...
    def __init__(self):
        ...
        self.septpointnew = 21
        self.septpointadjusted = 21
...
# CHOOSING AC SETPOINT IN AUTO MODE ------------------------------------------------------------------------------------
                ...
                self.septpointadjusted = (self.setpoint - self.overheatvalue)
                self.setpoint = self.septpointadjusted
                Domoticz.Debug("Overheat value is '{}' - New AC Setpoint after correction is '{}'".format(self.overheatvalue, self.septpoint))

                if self.PresenceTH:
                    if self.intemp < (float(Devices[5].sValue)):
                        self.overheat = False
                        if self.septpointadjusted < 17:
                            self.septpointadjusted = 17
                            self.setpoint = 17
                        Domoticz.Debug("NO Overheat - AC Setpoint without correction is '{}'".format(self.septpoint))
