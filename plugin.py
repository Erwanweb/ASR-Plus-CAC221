...
<plugin key="AC-ASR-CAC221" name="AC Aircon Smart Remote PLUS for CAC221" author="MrErwan" version="2.1.2" externallink="https://github.com/Erwanweb/ASR-Plus-CAC221.git">
    ...
    <params>
        ...
        <param field="Mode5" label="Day/Night Activator, Pause On delay, Forced Eco Off delay (0=not timed), Presence On delay, Presence Off delay (all in minutes), reducted T(in degree), Delta max fanspeed (in in tenth of degre), IR Repeat order (0 not activ.)" width="200px" required="true" default="0,1,1,2,45,3,10"/>
...
# CHOOSING AC SETPOINT IN AUTO MODE ------------------------------------------------------------------------------------
                ...
                if self.intemp > (float(Devices[5].sValue) + 0.2):
                    if self.intemp > (float(Devices[5].sValue) + 1.0):
                        self.overheatvalue = math.ceil((self.intemp - self.setpoint) +4)
                    elif self.intemp > (float(Devices[5].sValue) + 0.8):
                        self.overheatvalue = math.ceil((self.intemp - self.setpoint) +3)
                    elif self.intemp > (float(Devices[5].sValue) + 0.5):
                        self.overheatvalue = math.ceil((self.intemp - self.setpoint) +2)
                    elif self.intemp > (float(Devices[5].sValue) + 0.3):
...
# READING ROOM TEMP ----------------------------------------------------------------------------------------------------
        if self.nexttemps + timedelta(minutes=1) <= now:
            self.readTemps()
