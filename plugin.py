...
<plugin key="AC-ASR-CAC221" name="AC Aircon Smart Remote PLUS for CAC221" author="MrErwan" version="0.1.1" externallink="https://github.com/Erwanweb/ASR-Plus-CAC221.git">
    ...
    <params>
        ...
        <param field="Mode1" label="CAC221 widgets idx - AC mode - AC Fanspedd - AC Setpoint" width="100px" required="true" default=""/>
...
class BasePlugin:
    ...
    def onStart(self):
        ...
        if len(params1) == 3:
            self.WACmode = CheckParam("AC mode",params[0],0)
            Domoticz.Debug("AC mode widget idx = {}".format(self.WACmode))
            self.WACfanspeed = CheckParam("AC fan speed",params[1],0)
    ...
    def onCommand(self,Unit,Command,Level,Color):
        ...
        if (Unit == 1):
            ...
            if (Devices[1].sValue == "20"): # Mode auto
                ...
                for idx in self.self.WACmode:
                    DomoticzAPI("ype=command&param=switchlight&idx={}&switchcmd=Set%20Level&level=0".format(idx))
        ...
        if Unit in (2, 3, 5, 7):  # tout le reste
            self.onHeartbeat()
            ...
            if self.ModeAuto :
                if self.PresenceTH :
                    if self.intemp < (float(Devices[5].sValue) - ((self.deltamax / 10) + (self.deltamax / 20))):
                        ...
                        if not str(self.WACsetpointvalue) == str(self.setpoint):
                        ...
                        if not self.WACfanspeedvalue == 40:
                            for idx in self.WACfanspeed:
                                DomoticzAPI("type=command&param=switchlight&idx={}&switchcmd=Set%20Level&level=40".format(idx))
                    else :
                        ...
                        if self.intemp < (float(Devices[10].sValue) - (self.deltamax / 10)):
                            ...
                            if not self.WACfanspeedvalue == 40:
                                for idx in self.WACfanspeed:
                                    DomoticzAPI("ype=command&param=switchlight&idx={}&switchcmd=Set%20Level&level=40".format(idx))
                        else:
                            # AC Fan Speed Auto
                            ...
                            if not self.WACfanspeedvalue == 10:
                                for idx in self.WACfanspeed:
                                    DomoticzAPI("ype=command&param=switchlight&idx={}&switchcmd=Set%20Level&level=10".format(idx))
                ...
                if not self.WACmodevalue == Devices[2].sValue:
                    for idx in self.WACmode:
                            DomoticzAPI("type=command&param=switchlight&idx={}&switchcmd=Set%20Level&level={}".format(idx, Devices[2].sValue))
                if not self.WACfanspeedvalue == Devices[3].sValue:
                    for idx in self.WACfanspeed:
                            DomoticzAPI("type=command&param=switchlight&idx={}&switchcmd=Set%20Level&level={}".format(idx, Devices[3].sValue))
                if not str(self.WACsetpointvalue) == str(self.setpoint):
                    for idx in self.WACsetpoint:
                        DomoticzAPI("type=command&param=setsetpoint&idx={}&setpoint={}".format(idx, self.setpoint))
    ...
    def CAC221widgetcontrol(self):
        ...
        Domoticz.Debug("Checking widgets of CAC 221...")
        # check Mode
        APIjson = DomoticzAPI("type=devices&rid={}".format(self.WACmode))
        if APIjson:
        ...
        # check setpoint
        APIjson = DomoticzAPI("type=devices&rid={}".format(self.WACWACsetpoint))
        if APIjson:
           for device in APIjson["result"]:  # check if idx is ok
