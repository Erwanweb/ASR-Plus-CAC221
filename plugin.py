...
# CHOOSING AC SETPOINT IN AUTO MODE ------------------------------------------------------------------------------------
                ...
                if self.PresenceTH:
                    ...
                    if self.intemp < (float(Devices[5].sValue)- self.reductedsp):
                        ...
                        if self.setpoint < 17:
                            self.setpoint = 17
                        Domoticz.Debug("NO Overheat - No Presence - AC Reducted Setpoint without correction is '{}'".format(self.setpoint))
...
# NORMAL AUTO MODE -----------------------------------------------------------------------------------------------------
                    ...
                    if self.Turbofan:  # Turbofan is on
                        ...
                        if self.intemp > (float(Devices[5].sValue) - (self.deltamax / 50)):
                            self.Turbofan = False
